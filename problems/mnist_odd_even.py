...
def get_data_loader(dataset_name, dataroot, batch_size, val_ratio, world_size, rank, args, heterogeneity=0, num_workers=1, small=False):
    ...
    train_set = dataset(root=dataroot, train=True,
                        download=True, transform=transform_train, target_transform=transforms.Compose([ttransform]))
    val_set = dataset(root=dataroot, train=True,
                      download=True, transform=transform_test, target_transform=transforms.Compose([ttransform]))
